...
<?if($_REQUEST["MAPLAT"]):?> map.setCenter(new context.YMaps.GeoPoint(<?=$_REQUEST["MAPLAT"]?>), <?echo $arParams['INIT_MAP_SCALE']?>, context.YMaps.MapType.<?echo $arParams['INIT_MAP_TYPE']?>); <?endif?>
...
?>
	if (window.<?echo $arParams['ONMAPREADY2']?>)
	{
		<?
		...
		window.<?echo $arParams['ONMAPREADY2']?>();
		<?
		else:
		?>
		//alert('qq');
		window.<?echo $arParams['ONMAPREADY2']?>(map);
		<?
		endif;
		?>
	}
