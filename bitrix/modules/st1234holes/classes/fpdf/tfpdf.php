...
function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false)
{
	...
	$s=str_replace("\r",'',$txt);
	if ($this->unifontSubset) {
		$nb=mb_strlen($s, _ENCODE_);
		while($nb>0 && mb_substr($s,$nb-1,1,_ENCODE_)=="\n")	$nb--;
	}
	else {
		$nb=strlen($s);
		if($nb>0 && $s[$nb-1]=="\n")
	...
	while($i<$nb)
	{
		...
		if($c=="\n") {
			...
			if ($this->unifontSubset) {
				$this->Cell($w,$h,mb_substr($s,$j,$i-$j,_ENCODE_),$b,2,$align,$fill);
			}
			else {
		...
		if($l>$wmax)
		{
			...
			else
			{
				...
				if ($this->unifontSubset) {
					$this->Cell($w,$h,mb_substr($s,$j,$sep-$j,_ENCODE_),$b,2,$align,$fill);
				}
				else {
...
function Write($h, $txt, $link='')
{
	...
	while($i<$nb)
	{
		...
		if($c=="\n") {
			//Explicit line break
			if ($this->unifontSubset) {
				$this->Cell($w,$h,mb_substr($s,$j,$i-$j,_ENCODE_),0,2,'',0,$link);
			}
			else {
		...
		if($l>$wmax)
		{
			...
			{
				if ($this->unifontSubset) {
					$this->Cell($w,$h,mb_substr($s,$j,$sep-$j,_ENCODE_),0,2,'',0,$link);
				}
				else {
	...
	if($i!=$j) {
		if ($this->unifontSubset) {
			$this->Cell($l,$h,mb_substr($s,$j,$i-$j,_ENCODE_),0,0,'',0,$link);
		}
		else {
...
function Output($name='', $dest='')
{
	...
	switch($dest)
	{
		...
		case 'D':
			...
			if(ob_get_length())
				$this->Error('Some data has already been output, can\'t send PDF file');
			header('Content-Type: application/x-download; charset=cp1251');
			if(headers_sent())
				$this->Error('Some data has already been output, can\'t send PDF file');
			//header('Content-Length: '.strlen($this->buffer));
